...
/datum/action/cooldown/spell/healing/proc/do_healing(mob/living/cast_on, conditional_buff = FALSE, situational_bonus = 0)
	...
	if(healing_flags &= MIRACLE_HEAL_TARGET_ZONE)
		...
		for(var/obj/item/bodypart/B as anything in C.bodyparts)
			...
			B.heal_wounds(amount_healed * wound_modifier)
		C.update_damage_overlays()
	...
	if(.)
		...
		for(var/obj/O in oview(5, owner))
			if(is_type_in_typecache(O, natural_stuff))
				...
				energy_restoration = min(energy_restoration + 1, 10)
...
/datum/action/cooldown/spell/healing/abyssor/cast(mob/living/cast_on)
	...
	if(.)
		...
		if(istype(target_turf, /turf/open/water))
			...
			blood_restoration = 10 * W.water_level
		if(cast_on != owner)
		...
		if(situational_bonus > 0)
			conditional_buff = TRUE
			healing_flags |= MIRACLE_HEAL_OXY
		do_healing(cast_on, conditional_buff, situational_bonus)
...
/datum/action/cooldown/spell/healing/ravox/cast(mob/living/cast_on)
	...
	if(.)
		...
		if(length(target_wounds))
			...
			energy_restoration = length(target_wounds)
		do_healing(cast_on, conditional_buff, situational_bonus)
	...
	if(.)
		...
		if(iscarbon(cast_on))
			...
			if(C.health <= (C.maxHealth * 0.25))
				...
				situational_bonus = max(C.maxHealth * (0.001 ** health_percentage), 5)
		do_healing(cast_on, conditional_buff, situational_bonus)
...
/datum/action/cooldown/spell/healing/xylix/cast(mob/living/cast_on)
	...
	if(.)
		...
		if(prob(50))
			...
			if(prob(50))
				healing_flags |= MIRACLE_HEAL_TOX
			if(prob(50))
				healing_flags |= MIRACLE_HEAL_OXY
		do_healing(cast_on, conditional_buff, situational_bonus)
...
/datum/action/cooldown/spell/healing/malum/cast(mob/living/cast_on)
	...
	if(.)
		...
		for(var/obj/machinery/light/fueled/O in oview(5, owner))
			...
			energy_restoration = min(energy_restoration + 5, max((cast_on.max_energy * 0.2) - cast_on.energy, 0))
		if(situational_bonus > 0)
			conditional_buff = TRUE
		do_healing(cast_on, conditional_buff, situational_bonus)
