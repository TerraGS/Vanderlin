...
/datum/component/echolocation/Initialize(echo_range, cooldown_time, image_expiry_time, fade_in_time, fade_out_time, images_are_static, blocking_trait, echo_group, echo_icon = "echo", color_path)
	...
	echolocator.add_traits(list(TRAIT_ECHOLOCATION_RECEIVER), echo_group) //so they see all the tiles they echolocated, even if they are in the dark
...
/datum/component/echolocation/Destroy(force)
	...
	echolocator.remove_traits(list(TRAIT_ECHOLOCATION_RECEIVER), echo_group)
...
/datum/component/echolocation/proc/generate_appearance(atom/input)
	...
	if(iscarbon(input))
		copied_appearance.appearance = copy_appearance_filter_overlays(input)
	...
	if(use_outline && !iscarbon(input))
		copied_appearance.filters += outline_filter(size = 2, color = COLOR_WHITE)
	if(!images_are_static)
